...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Create ContactEmail objects."""
        for index, email in enumerate(orm['contacts.Email'].objects.all()):
            orm['contacts.ContactEmail'](contact=email.contact,
                email=email, order=index+1).save()

    def backwards(self, orm):
        """Remove ContactEmail objects."""
        for contact_email in orm['contacts.ContactEmail'].objects.all():
            email = contact_email.email
            email.contact = contact_email.contact
            email.save()
