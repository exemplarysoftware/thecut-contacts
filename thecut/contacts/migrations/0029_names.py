...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for person in orm['contacts.Person'].objects.all():
            person.short_name = person.first_name
            person.long_name = ' '.join([name for name in [
                person.first_name or '', person.last_name or ''] if name])
            person.save()

    def backwards(self, orm):
        for person in orm['contacts.Person'].objects.all():
            person.first_name = person.short_name
            person.last_name = ' '.join(person.long_name.split()[1:])
            person.save()
