...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Create ContactInstantMessengerHandle objects."""
        for index, handle in enumerate(orm[
            'contacts.InstantMessengerHandle'].objects.all()):
            orm['contacts.ContactInstantMessengerHandle'](
                contact=handle.contact, instant_messenger_handle=handle,
                order=index+1).save()

    def backwards(self, orm):
        """Remove ContactInstantMessengerHandle objects."""
        for contact_handle in orm[
            'contacts.ContactInstantMessengerHandle'].objects.all():
            handle = contact_handle.instant_messenger_handle
            handle.contact = contact_handle.contact
            handle.save()
